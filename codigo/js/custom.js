	...
	for ( var i = 0; i < linkExclusao.length; i++ ) {
		(function(i){
			...
			if (linkExclusao[i].addEventListener){
		    	linkExclusao[i].addEventListener("click", function(){confirmaExclusao_cl(id_cliente);});
			}else if (linkExclusao[i].attachEvent) { 
				linkExclusao[i].attachEvent("onclick", function(){confirmaExclusao_cl(id_cliente);});
			}
		})(i);
	}
}
...
function confirmaExclusao_cl(cpf){
...
function confirmaExclusao_ag(id){
	...
	if (retorno){
	    ...
	    //Cria um formulário
	    var formulario = document.createElement("form");
	    formulario.action = "action_agendar.php";
	    formulario.method = "post";
